...
class CoronaDebuggerThread(threading.Thread):
  ...
  def stop(self):
    debug("CoronaDebuggerThread: stop")
    self.debugger_running = False
  ...
  def run(self):
    ...
    # self.socket.settimeout(10) # TODO: figure out the best way to handle the socket not being connected
    debug("Socket about to accept")
    while True:
      try:
        self.conn, addr = self.socket.accept()
      except socket.error as msg:
        debug( 'Accept failed: ' + str(msg))
      else:
        debug( 'Socket accepted' )
        break

    # self.socket.settimeout(None) # Revert to no timeout once things are established
    ...
    bpMatches = re.search(r'^202 Paused\s+([\w,.]+)\s+(\d+)$', bpResponse.strip())
    ...
    # setting breakpoint before it's started)
  ...
  def doContinue(self, cmd):
    ...
    self.getAck(cmd)
    response = self.readFromPUT().strip()
    if response is None or response == "":
      debugger_status("Program finished")
      self.debugger_running = False
      return

    statusMatches = re.search(r'^(\d+)', response)
    status = statusMatches.group(0)
    debug("Status: " + status)
    if status == "202":
      bpMatches = re.search(r'^202 (\w+)\s+([\w,.]+)\s+(\d+)$', response)
      if bpMatches is not None:
      ...
      bpwMatches = re.search(r'^203 Paused\s+([\w,.]+)\s+(\d+)\s+(\d+)$', response)
      if bpwMatches is not None:
        file = bpwMatches.group(1)
        line = bpwMatches.group(2)
        watchIndex = bpwMatches.group(3)
        if file and line and watch_idx:
          debugger_status("Paused at file " + file + " line " + line + " (watch expression " + watch_idx + ": [" + watches[watch_idx] + "])")
      else:
...
class CoronaDebuggerCommand(sublime_plugin.WindowCommand):
  ...
  def run(self, cmd = None, arg_filename = None, arg_lineno = None, arg_toggle = True):
    ...
    if coronaDbg == None and cmd in ['run', 'step', 'over']:
      cmd = "start"
    ...
    if cmd == "start":
      ...
      dbg_path, dbg_flags = _corona_utils.GetSimulatorCmd(True)
      dbg_cmd = [ dbg_path ]
      dbg_cmd += dbg_flags
  ...
  def dumpVariable(self):
    ...
    if selection:
      ...
      word = sublime.active_window().show_input_panel("Variable name or expression:", "", self.doDumpVariable, None, None)
...
def RunSubprocess(cmd):
  ...
  coronaDbgThread = CoronaSubprocessThread(1, cmd)
